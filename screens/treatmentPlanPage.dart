...
class _TreatmentPlanState extends State<TreatmentPlan> {
  ...
  Future<void> fetchTreatmentPlan() async {
    ...
    final url = 'http://localhost:5000/getTreatmentPlan/$userId';
  ...
  print('Response data: $data'); // Add this line to print the data to the console

  setState(() {
    diagnosesText = data['treatmentPlan']['diagnoses'] ?? 'No diagnoses';
    tasks = List<Task>.from(data['treatmentPlan']['tasks']?.map((x) => Task.fromJson(x)) ?? []);
    ...
    final int? userId = await getUserId();
    final url = 'http://localhost:5000/hasEmojiForToday/$userId';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                     ...
                     onPressed: navigateToMoodPage, // Use the navigateToMoodPage function
                  ...
                  child: Text("Mood Tracking", style: TextStyle(color: Colors.white)),
                ),
                ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                  ...
                  child: Text("Upcoming Sessions", style: TextStyle(color: Colors.white)),
                ),
              ],
            ),
             SizedBox(height: 20),
      ...
      child: Container(
        height: 2.0,
        width: 80.0, // Adjust the width of the line as needed
        ...
        child: Padding(
          ...
          child: Text(
            ...
            Padding(
              ...
              child: Column(
                children: tasks.map((task) => buildTaskContainer(task)).toList(),
              ),
            ),
          ],
        ),
